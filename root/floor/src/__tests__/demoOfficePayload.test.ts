...
describe('buildDemoOfficePayload — chrome per floor', () => {
  ...
  it('includes premium props with export-safe asset metadata', () => {
    const premiumTypes = new Set(['monitor', 'task-lamp', 'credenza', 'bookshelf', 'area-rug'])
    const premiumProps = payload.floors.flatMap((floor) =>
      Object.values(floor.elements).filter((e) => premiumTypes.has(e.type)),
    )

    expect(premiumProps.length).toBeGreaterThanOrEqual(5)
    expect(new Set(premiumProps.map((e) => e.type))).toEqual(premiumTypes)
    for (const prop of premiumProps) {
      expect(prop.assetMetadata).toBeTruthy()
      expect(prop.assetMetadata?.catalogId).toBe(`built-in/${prop.type}`)
      expect(prop.assetMetadata?.exportSafe).toBe(true)
    }
  })
})
