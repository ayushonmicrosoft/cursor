...
describe('buildLegend', () => {
  ...
  it('uses friendly labels for premium furniture props', () => {
    const elements: CanvasElement[] = [
      desk('m1', { type: 'monitor' }),
      desk('l1', { type: 'task-lamp' }),
      desk('c1', { type: 'credenza' }),
      desk('b1', { type: 'bookshelf' }),
      desk('r1', { type: 'area-rug' }),
    ]
    const labels = Object.fromEntries(buildLegend(elements).map((entry) => [entry.type, entry.label]))

    expect(labels.monitor).toBe('Monitor')
    expect(labels['task-lamp']).toBe('Task lamp')
    expect(labels.credenza).toBe('Credenza')
    expect(labels.bookshelf).toBe('Bookshelf')
    expect(labels['area-rug']).toBe('Area rug')
  })
