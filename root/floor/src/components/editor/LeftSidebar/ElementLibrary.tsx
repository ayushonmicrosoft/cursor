...
import type {
  ...
  FurnitureAssetSource,
  FurnitureAssetLicense,
} from '../../../types/elements'
...
export interface LibraryItem {
  ...
  tags?: string[]
  materialHint?: string
  assetSource?: FurnitureAssetSource
  license?: FurnitureAssetLicense
  sourceName?: string
  sourceUrl?: string
...
const TILE_DESCRIPTIONS: Record<string, string> = {
  ...
  monitor: 'Desk monitor accessory for higher-fidelity workstations.',
  'task-lamp': 'Task lamp accessory for desks, reception, or lounge tables.',
  credenza: 'Low storage credenza for executive offices and meeting rooms.',
  bookshelf: 'Wall-side bookshelf / storage run for finished office plans.',
  'area-rug': 'Soft-zone rug that grounds lounge or executive layouts.',
...
function assetMetadataFor(item: LibraryItem): FurnitureAssetMetadata | undefined {
  if (!item.qualityTier && !item.tags && !item.materialHint && !item.assetSource) {
    return undefined
  }

  return {
    catalogId: item.catalogId ?? `built-in/${tileKey(item)}`,
    qualityTier: item.qualityTier ?? 'standard',
    assetSource: item.assetSource ?? 'built-in-vector',
    tags: item.tags ?? [],
    materialHint: item.materialHint ?? '',
    exportSafe: true,
    license: item.license ?? 'built-in',
...
const LIBRARY_ITEMS: LibraryItem[] = [
  ...
  // Seating
  { type: 'chair',             label: 'Office Chair',    category: 'Seating', catalogId: 'built-in/office-chair', qualityTier: 'premium-ready', tags: ['ergonomic', 'seating', 'office'], materialHint: 'fabric + metal base', assetSource: 'built-in-vector' },
  ...
  // Premium planning props: vector-backed now, ready to map to GLB assets
  // once the app gets a 3D asset pipeline.
  { type: 'monitor',           label: 'Monitor',         category: 'Premium Props', catalogId: 'built-in/monitor', qualityTier: 'premium-ready', tags: ['screen', 'computer', 'desk', 'office'], materialHint: 'black glass + metal stand', assetSource: 'glb-ready' },
  { type: 'task-lamp',         label: 'Task Lamp',       category: 'Premium Props', catalogId: 'built-in/task-lamp', qualityTier: 'premium-ready', tags: ['lighting', 'desk', 'lamp', 'office'], materialHint: 'brass shade + dark arm', assetSource: 'glb-ready' },
  { type: 'credenza',          label: 'Credenza',        category: 'Premium Props', catalogId: 'built-in/credenza', qualityTier: 'premium-ready', tags: ['storage', 'executive', 'cabinet', 'wood'], materialHint: 'warm wood veneer + handles', assetSource: 'glb-ready' },
  { type: 'bookshelf',         label: 'Bookshelf',       category: 'Premium Props', catalogId: 'built-in/bookshelf', qualityTier: 'premium-ready', tags: ['storage', 'library', 'wall', 'shelves'], materialHint: 'wood shelving + book blocks', assetSource: 'glb-ready' },
  { type: 'area-rug',          label: 'Area Rug',        category: 'Premium Props', catalogId: 'built-in/area-rug', qualityTier: 'standard', tags: ['rug', 'soft zone', 'lounge', 'executive'], materialHint: 'woven textile', assetSource: 'built-in-vector' },
...
export function buildLibraryElement(
  ...
  const id = nanoid()
  const assetMetadata = assetMetadataFor(item)
  ...
  const matchesQuery = useCallback(
    (i: LibraryItem) => {
      ...
      return (
        ...
        (i.tags?.some((tag) => tag.toLowerCase().includes(q)) ?? false) ||
        (i.materialHint?.toLowerCase().includes(q) ?? false) ||
        (i.qualityTier?.toLowerCase().includes(q) ?? false)
      )
    },
    [isSearching, q],
  )
