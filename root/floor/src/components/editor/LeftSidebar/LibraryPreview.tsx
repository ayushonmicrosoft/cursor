...
export function LibraryPreview({ item }: Props) {
  ...
  if (item.type === 'monitor') {
    return (
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} aria-hidden>
        <rect x={5} y={3} width={14} height={8} rx="1.5" fill={fill} stroke={stroke} />
        <path d="M8 6h8" stroke="#38BDF8" strokeWidth="1" opacity="0.65" />
        <path d="M12 11v3m-4 1h8" stroke={stroke} strokeWidth="1.2" strokeLinecap="round" />
      </svg>
    )
  }

  if (item.type === 'task-lamp') {
    return (
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} aria-hidden>
        <circle cx={7} cy={14} r={2.2} fill={stroke} opacity="0.3" />
        <path d="M7 13l4-6 5-3" stroke={stroke} strokeWidth="1.4" fill="none" strokeLinecap="round" />
        <rect x={15} y={2} width={6} height={4} rx="1.5" fill={fill} stroke={stroke} transform="rotate(-18 18 4)" />
      </svg>
    )
  }

  if (item.type === 'credenza') {
    return (
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} aria-hidden>
        <rect x={2} y={5} width={20} height={8} rx="2" fill={fill} stroke={stroke} />
        {[6, 12, 18].map((x) => <circle key={x} cx={x} cy={9} r="0.8" fill={stroke} opacity="0.7" />)}
        <path d="M9 5v8m6-8v8" stroke={stroke} opacity="0.5" />
      </svg>
    )
  }
  ...
  if (item.type === 'bookshelf') {
    return (
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} aria-hidden>
        <rect x={3} y={4} width={18} height={10} rx="1.5" fill={fill} stroke={stroke} />
        <path d="M5 7h14M5 10h14" stroke="#FDECC8" strokeWidth="1" />
        <path d="M7 5v4m3-4v3m5-3v4m2-4v3" stroke="#2563EB" strokeWidth="1.2" opacity="0.85" />
      </svg>
    )
  }

  if (item.type === 'area-rug') {
    return (
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} aria-hidden>
        <rect x={2} y={3} width={20} height={12} rx="3" fill={fill} stroke={stroke} />
        <rect x={5} y={5} width={14} height={8} rx="2" fill="none" stroke="#fff" strokeDasharray="2 2" opacity="0.75" />
      </svg>
    )
  }
