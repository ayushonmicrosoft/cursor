...
type PremiumPropType = 'monitor' | 'task-lamp' | 'credenza' | 'bookshelf' | 'area-rug'

function assetMetadata(
  catalogId: string,
  materialHint: string,
  tags: string[],
  assetSource: FurnitureAssetMetadata['assetSource'] = 'glb-ready',
): FurnitureAssetMetadata {
  return {
    catalogId,
    qualityTier: assetSource === 'glb-ready' ? 'premium-ready' : 'standard',
    assetSource,
    tags,
    materialHint,
    exportSafe: true,
    license: 'built-in',
    sourceName: 'OandOcraft built-in vector catalog',
  }
}
...
function makePlant(x: number, y: number): PlantElement {
  return {
    ...
    assetMetadata: assetMetadata(
      'built-in/plant',
      'leaf clusters + ceramic pot',
      ['plant', 'decor', 'biophilic'],
      'built-in-vector',
    ),
  }
}
...
function makeSofa(x: number, y: number, width = 120, height = 56): SofaElement {
  return {
    ...
    assetMetadata: assetMetadata(
      'built-in/sofa',
      'soft grey upholstery + dark arms',
      ['sofa', 'lounge', 'seating'],
      'built-in-vector',
    ),
  }
...
function makePremiumProp(
  type: PremiumPropType,
  x: number,
  y: number,
  width: number,
  height: number,
  ...
  return {
    ...
    label,
    visible: true,
    style: baseStyle(fill, stroke),
    assetMetadata: assetMetadata(`built-in/${type}`, materialHint, tags, type === 'area-rug' ? 'built-in-vector' : 'glb-ready'),
  } as CanvasElement
}
...
function buildGroundFloor(): FloorBuild {
  ...
  els.push(makePremiumProp('area-rug', 170, 92, 170, 72, 'Reception rug', '#DBEAFE', '#2563EB', 'woven blue textile', ['rug', 'reception', 'soft-zone']))
  els.push(makeSofa(170, 95, 140, 40))
  els.push(makePremiumProp('task-lamp', 252, 74, 36, 36, 'Reception lamp', '#FACC15', '#854D0E', 'brass shade + dark arm', ['lamp', 'lighting', 'reception']))
  ...
  els.push(makePremiumProp('monitor', 146, 320, 42, 26, 'Ops monitor', '#0F172A', '#475569', 'black glass + metal stand', ['monitor', 'screen', 'private-office']))
  els.push(makePremiumProp('monitor', 146, 450, 42, 26, 'People monitor', '#0F172A', '#475569', 'black glass + metal stand', ['monitor', 'screen', 'private-office']))
...
function buildEngineeringFloor(): FloorBuild {
  ...
  els.push(makePremiumProp('bookshelf', 1030, 92, 90, 28, 'Reference shelf', '#B7791F', '#6B3F16', 'wood shelving + book blocks', ['bookshelf', 'storage', 'engineering']))
  els.push(makePremiumProp('credenza', 800, 720, 150, 46, 'Team credenza', '#D6B884', '#7C4A1D', 'warm wood veneer + handles', ['storage', 'credenza', 'office']))
