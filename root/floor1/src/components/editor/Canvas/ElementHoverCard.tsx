...
function buildCardData(args: {
  ...
  if (isWallElement(el)) {
    ...
    const thickness = toRealLength(el.thickness, scale, scaleUnit)
    return {
      kind: 'summary',
      Icon: Minus,
      typeLabel: 'Wall',
      primary: el.label || '',
      secondary: `${formatLength(real, scaleUnit)} ${scaleUnit} length • ${formatLength(thickness, scaleUnit)} ${scaleUnit} thick`,
    }
  }
