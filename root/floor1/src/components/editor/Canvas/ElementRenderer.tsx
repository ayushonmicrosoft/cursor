...
import { getSnappedPosition } from '../../../lib/geometry'
...
export function ElementRenderer() {
  ...
  const handleDragMove = useCallback(
    (id: string, e: Konva.KonvaEventObject<DragEvent>) => {
      ...
      const gridSize = useCanvasStore.getState().settings.gridSize
      const showGrid = useCanvasStore.getState().settings.showGrid

      let finalTopLeftX = snapped.x
      let finalTopLeftY = snapped.y

      // Fallback to grid snapping if no alignment guide matched
      if (showGrid) {
        if (snapped.x === topLeft.x) finalTopLeftX = Math.round(topLeft.x / gridSize) * gridSize
        if (snapped.y === topLeft.y) finalTopLeftY = Math.round(topLeft.y / gridSize) * gridSize
      }

      e.target.x(finalTopLeftX + w / 2)
      e.target.y(finalTopLeftY + h / 2)
  ...
  const handleDragEnd = useCallback(
    (id: string, e: Konva.KonvaEventObject<DragEvent>) => {
      const x = e.target.x()
      const y = e.target.y()
      // Note: we removed the greedy post-drop grid snap here because it was clobbering
      // the high-precision alignment guides calculated during handleDragMove.
      updateElement(id, { x, y })
      clearDragAlignmentGuides()
    },
    [updateElement, clearDragAlignmentGuides]
  )
            ...
            return (
              <Group rotation={el.rotation} listening={!el.locked}>
                <Group x={-el.width / 2} y={-el.height / 2}>
                  {/* Invisible hit rect so the whole bounds are clickable even if shape is hollow */}
