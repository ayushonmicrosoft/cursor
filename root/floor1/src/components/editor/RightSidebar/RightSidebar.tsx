...
import { AlertTriangle, BarChart3, Settings, Users, Shield } from 'lucide-react'
...
type TabId = 'properties' | 'people' | 'reports' | 'insights' | 'admin'

export function RightSidebar() {
  const tab = useUIStore((s) => s.rightSidebarTab as TabId)
  const setTab = useUIStore((s) => s.setRightSidebarTab)

  const session = useSession()
  const isSuperAdmin = session.status === 'authenticated' && session.user.email === 'ayush@oando.co.in'
  ...
  if (isSuperAdmin) {
    tabs.push({ id: 'admin', label: 'Admin', icon: <Shield size={14} aria-hidden="true" /> })
  }
