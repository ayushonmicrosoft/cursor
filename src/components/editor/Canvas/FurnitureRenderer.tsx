...
function ChairSymbol({ width: w, height: h, fill, stroke, strokeWidth, opacity }: SymbolProps) {
  ...
  const r = Math.min(3, Math.min(w, h) * 0.08)
  return (
    <>
      <Rect
        x={-seatW / 2}
      ...
      <Rect
        ...
        cornerRadius={Math.min(2, backH / 4)}
        opacity={opacity}
      />
      <Rect x={-seatW / 2 - armW} y={-seatH / 2 + backH * 0.25} width={armW} height={seatH * 0.9} fill="#EFF6FF" stroke={stroke} strokeWidth={1} cornerRadius={1} opacity={opacity} />
      <Rect x={seatW / 2} y={-seatH / 2 + backH * 0.25} width={armW} height={seatH * 0.9} fill="#EFF6FF" stroke={stroke} strokeWidth={1} cornerRadius={1} opacity={opacity} />
...
function CounterSymbol({ width: w, height: h, fill, stroke, strokeWidth, opacity }: SymbolProps) {
  ...
  return (
    <>
      <Rect x={-w / 2} y={-h / 2} width={w} height={h} fill={fill} stroke={stroke} strokeWidth={strokeWidth} cornerRadius={2} opacity={opacity} />
      <Rect x={-w / 2 + inset} y={-h / 2 + inset} width={w - inset * 2} height={h - inset * 2} fill="#FFFFFF" stroke={stroke} strokeWidth={1} cornerRadius={1} opacity={opacity * 0.8} listening={false} />
...
function DividerSymbol({ width: w, height: h, stroke, strokeWidth, opacity }: SymbolProps) {
  ...
  return (
    <>
      <Rect x={-w / 2} y={-railH / 2} width={w} height={railH} fill="#E2E8F0" stroke={stroke} strokeWidth={strokeWidth} cornerRadius={1} opacity={opacity} />
...
function PlanterSymbol({ width: w, height: h, fill, stroke, strokeWidth, opacity }: SymbolProps) {
  ...
  return (
    <>
      ...
      <Rect x={-w * 0.24} y={h * 0.2} width={w * 0.48} height={Math.max(5, h * 0.16)} fill="#A16207" stroke={stroke} strokeWidth={Math.max(1, strokeWidth * 0.5)} cornerRadius={[0, 0, 2, 2]} opacity={opacity} listening={false} />
    </>
  )
}
...
function GenericBlockSymbol({ width: w, height: h, fill, stroke, strokeWidth, opacity }: SymbolProps) {
  ...
  return (
    <>
      ...
      <Rect x={-w * 0.34} y={-h * 0.24} width={w * 0.68} height={h * 0.18} fill="#FFFFFF" opacity={opacity * 0.45} cornerRadius={1} listening={false} />
