...
export function AccountPage() {
  ...
  const subtitle = name.trim() ? email : 'OandOcraft account'
    ...
    >
      ...
      >
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300">
          ...
          address. Contact support from your current email to start the
          process — we'll move your account, team
...
function DataPrivacySection() {
  ...
  async function onExport() {
    ...
    try {
      ...
      a.href = url
      a.download = `oandocraft-data-${new Date().toISOString().slice(0, 10)}.json`
      ...
      <div className="rounded-xl border border-red-200 bg-red-50/50 dark:border-red-900 dark:bg-red-950/20 divide-y divide-red-200/70 dark:divide-red-900/60">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-5">
          <div>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">
              Sign out of OandOcraft on this device. Other browsers
              stay signed in.
            </div>
          </div>
          <Button
