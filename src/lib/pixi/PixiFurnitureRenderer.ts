...
export function renderFurniture(g: Graphics, el: CanvasElement, selected: boolean): void {
  ...
  const sColor = selected ? PIXI_COLORS.selected : stroke
  ...
  // Try to render silhouette first
  const silhouetteImg = getSilhouetteImage(el.type)
  if (silhouetteImg) {
    // Create a texture from the silhouette image
    const tempCanvas = document.createElement('canvas')
    tempCanvas.width = silhouetteImg.width
    tempCanvas.height = silhouetteImg.height

    const tempCtx = tempCanvas.getContext('2d')
    if (tempCtx) {
      // Draw the silhouette image to the temporary canvas
    ...
    if (t === 'sofa') {
      ...
      g.roundRect((w - innerCushionW) / 2, innerPadY, innerCushionW, backrestH, 1)
        .fill({ color: PIXI_COLORS.sofaCushion, alpha: 0.4 })
        .stroke({ color: sColor, width: sw * 0.8 })
      ...
      // Foliage
      g.circle(w / 2, foliageR, foliageR).fill({ color: fill }).stroke({ color: selected ? PIXI_COLORS.selected : PIXI_COLORS.plantStroke, width: sw })
      // Pot base
      ...
      if (shape === 'storage') {
         ...
         // Printer silhouette
         g.roundRect(w * 0.15, h * 0.15, w * 0.7, h * 0.6, 1).fill({ color: PIXI_COLORS.sofaCushion, alpha: 0.4 }).stroke({ color: stroke, width: 0.5 })
