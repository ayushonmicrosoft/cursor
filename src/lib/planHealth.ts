    ...
    if (emp.seatId) {
      ...
      if (!hit) {
        issues.push({
          ...
          severity: 'error',
          category: 'reference',
          message: `An employee is assigned to a deleted desk`,
      ...
      if (isDeskElement(el)) {
        if (el.assignedEmployeeId && !employees[el.assignedEmployeeId]) {
          issues.push({
            ...
            severity: 'error',
            category: 'reference',
            message: `A desk references a deleted employee`,
        ...
        if (el.type === 'hot-desk' && el.assignedEmployeeId) {
          issues.push({
            ...
            severity: 'warning',
            category: 'reference',
            message: `A hot-desk has a persistent assignee`,
      ...
      if (isWorkstationElement(el)) {
        ...
        if (occupants.length > el.positions) {
          issues.push({
            ...
            severity: 'error',
            category: 'capacity',
            message: `A workstation is over capacity`,
        ...
        if (stale.length > 0) {
          issues.push({
            ...
            severity: 'error',
            category: 'reference',
            message: `A workstation references deleted employees`,
      ...
      if (isPrivateOfficeElement(el)) {
        ...
        if (ids.length > el.capacity) {
          issues.push({
            ...
            severity: 'error',
            category: 'capacity',
            message: `A private office is over capacity`,
        ...
        if (stale.length > 0) {
          issues.push({
            ...
            severity: 'error',
            category: 'reference',
            message: `A private office references deleted employees`,
            detail: `${stale.length} stale employee id${stale.length === 1 ? '' : 's'}`,
            floorId,
            targetIds: [el.id],
          })
